...
참외밭을 이루는 육각형의 임의의 한 꼭짓점에서 출발하여 반시계방향으로 둘레를 돌면서 지나는 변의 방향과 
길이가 순서대로 주어진다. 이 참외밭에서 자라는 참외의 수를 구하는 프로그램을 작성하시오.
...
for d, l in sides:
    if d == 1 or d == 2:   # 동(1), 서(2) -> 가로
        widths.append(l)
    elif d == 3 or d == 4: # 남(3), 북(4) -> 세로
        heights.append(l)
...
for i in range(6):
    d, l = sides[i]
    if l == max_w and d in (1, 2):
        iw = i
    if l == max_h and d in (3, 4):
        ih = i

# +3 하는 이유: 어떤 변의 인덱스가 i라면 세 칸뒤는 항상 반대편에 있는 변이고 서로 직각
small_h = sides[(ih+3)%6][1] # %6은 인덱스 범위 넘어가는거 방지
small_w = sides[(iw+3)%6][1]

korean_mellon = (max_h * max_w - small_h * small_w) * K
print(korean_mellon)
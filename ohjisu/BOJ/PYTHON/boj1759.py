...
def make_pswd(start, v_cnt, now_pswd) :
    if len(now_pswd) == L :
        if 1 <= v_cnt <= (L - 2) : # 모음 1개 이상, (전체 - 최소 자음 개수(2))개 이하
            print(now_pswd)

        return
    for i in range(start, len(default_arr)) :
        if default_arr[i] in 'aeiou' : # 모음이면  v_cnt + 1
            make_pswd(i + 1, v_cnt + 1, now_pswd + default_arr[i])
        else :                         # 자음이면 그대로 넘기기기
            make_pswd(i + 1, v_cnt, now_pswd + default_arr[i])



L, C = map(int, input().split())
default_arr = sorted(list(input().split())) # 오름차순으로 미리 sort
make_pswd(0, 0, '')

...
if K >= N : # 센서보다 집중국이 더 많으면 거리는 0
    ...
    dist = []
    for idx in range(N - 1) : # 거리 리스트 만들기
        dist.append(arr[idx + 1] - arr[idx])

    dist.sort(reverse=True) # 큰 거리 제거를 위해 정렬

    for _ in range(K - 1) : # K - 1번 반복하면서 큰 거리 제거하기
        dist.pop(0)

    print(sum(dist)) # 남은 거리 더하기
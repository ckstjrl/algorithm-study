...
def find(x):
    global parent # UnboundLocalError 방지
    while parent[x] != x:
...
def union(a, b):
    global parent, size # UnboundLocalError 방지
    ra, rb = find(a), find(b)
    if ra == rb:
        return
    # size 기준 병합 (작은 트리를 큰 트리에 붙이기)
    if size[ra] < size[rb]:
        ra, rb = rb, ra
    parent[rb] = ra
    size[ra] += size[rb]
...
'''
bfs로 풀이 진행했다가 시간 초과 발생
union-find로 풀이 진행
여러가지 오류들이 발생했고 하나씩 해결해가며 문제 풀이 진행
'''
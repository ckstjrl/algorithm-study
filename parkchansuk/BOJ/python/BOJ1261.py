...
dx = [0, 0, -1, 1]
def dijkstra():
    dists = [[21e8] * M for _ in range(N)] # 최단거리를 저장할 이차원 배열
    dists[0][0] = 0
    ...
    while pq:
        ...
        for i in range(4):
            ...
            if ny < 0 or nx < 0 or ny >= N or nx >= M:
                continue
            # 누적 거리 계산 = 이미 온 거리 + 추가 거리
            next_dist = dist + arr[ny][nx]
            # 이미 온 거리가 누적 거리보다 작은 경우 무시
            if dists[ny][nx] <= next_dist:
                continue
...
'''
수업 시간에 배운 이차원 배열의 다익스트라 방법을 활용하면 된다.
'''
...
if K > C * R:   # # 좌석 수보다 K가 크면 배정 불가.
    ...
    if K == 1:  # K가 1이면 바로 출력하자.
        print(x, y)
    else:
        while current_order < C * R:
            nx = x + dx[dir_idx]
            ny = y + dy[dir_idx]

            out_of_range = False    # 범위 밖이거나 이미 방문이면 방향 전환하자.
            if nx < 1 or nx > C or ny < 1 or ny > R:
                out_of_range = True
            else:
            ...
            if current_order == K:  # 딱 K번째면 출력하고 종료.
                print(x, y)
                break
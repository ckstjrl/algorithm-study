...
여러분은 지우를 도와 줄 프로그램을 작성하고자 한다. 즉 설문조사 결과대로 호텔 정문으로부터 걷는 거리가 가장 짧도록 방을 배정하는 프로그램을 작성하고자 한다.
...
for i in range(T):
    ...
    room = n//h + 1  # (손님) 나누기 (층 수) + 1
    floor = n % h    # 손님 나누기 층

    if n % h == 0: # h의 배수이면, => 위에서 나눈 n//h 나머지 몫이 0인 때 조건식
        room = n//h
        floor = h

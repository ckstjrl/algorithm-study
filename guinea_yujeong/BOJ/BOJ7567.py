...
height = 10 # 그릇의 높이, 그릇 1개를 기준 높이로 설정

for i in range(1, len(dish)): # a[1]부터 시작하는 for문 입력된 그릇 갯수 만큼 동작
    if dish[i] == dish[i-1]: # 이전과 그릇의 방향이 같을 경우
        height += 5    # 5 증가
    else:               # 그릇의 방향이 다르다면
        height += 10    # 10 증가
...
# 메모 '(' <- 이 문자를 따로 입력하지 않고 답을 도출할 수 있는 방법을 떠올리지 못한게 아쉬움